(() => {
  ...
  const next = index < files.length - 1 ? `<a href="${files[index+1]}" aria-label="Laboratorio siguiente">Siguiente →</a>` : `<a href="../evaluacion/cuestionario-bitcoin.html">Cuestionario →</a>`;
  dock.innerHTML = `${previous}<div class="lab-position" data-short="${index+1}/9">Laboratorio ${index+1} de 9</div><button type="button" data-complete>${progress[current] ? "Completada ✓" : "Marcar completada"}</button>${next}`;
